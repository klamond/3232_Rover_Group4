...
// X and Y are swapped in this code but it works so i dont want to fuck with it
void movement_parse_502(void) {
    uint16_t Raw_X_Axis = ((uint16_t)comms_payload[1] << 8) | comms_payload[0];  // Forward/Reverse
    uint16_t Raw_Y_Axis = ((uint16_t)comms_payload[3] << 8) | comms_payload[2];  // Steering

    int16_t X_Centered = (int16_t)(Raw_X_Axis - 1500);  // Center at 0, range -500 to +500
    int16_t Y_Centered = (int16_t)(Raw_Y_Axis - 1500);
